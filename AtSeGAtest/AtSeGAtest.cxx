#include "AtSeGAtest.h"

#include "AtDetectorList.h"
#include "AtSeGAtestPoint.h"
#include "AtStack.h"
...
AtSeGAtest::AtSeGAtest()
   : FairDetector("AtSeGAtest", kTRUE, kAtSeGAtest), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.),
     fLength(-1.), fELoss(-1), fPosIndex(-1), fAtSeGAtestPointCollection(new TClonesArray("AtSeGAtestPoint")), fELossAcc(-1)
{
   // LOG(INFO)<<" AtSeGAtest detector initialized ";
}

AtSeGAtest::AtSeGAtest(const char *name, Bool_t active)
   : FairDetector(name, active, kAtSeGAtest), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fPosIndex(-1), fAtSeGAtestPointCollection(new TClonesArray("AtSeGAtestPoint")), fELossAcc(-1)
{
   // LOG(INFO)<<" AtSeGAtest detector initialized ";
}
...
AtSeGAtest::~AtSeGAtest()
{
   if (fAtSeGAtestPointCollection) {
      fAtSeGAtestPointCollection->Delete();
      delete fAtSeGAtestPointCollection;
   }
}

void AtSeGAtest::Initialize()
{
   FairDetector::Initialize();
   // FairRuntimeDb *rtdb = FairRun::Instance()->GetRuntimeDb();
   // auto *par = (AtSeGAtestGeoPar *)(rtdb->getContainer("AtSeGAtestGeoPar"));
}

Bool_t AtSeGAtest::ProcessHits(FairVolume *vol)
{
   ...
   LOG(debug) << "In AtSeGAtest::ProcessHits";
   ...
   if (TVirtualMC::GetMC()->IsTrackEntering()) {
      ...
      std::cout << " AtSeGAtest: Track is entering "
                << "\n";
   ...
   // Create AtSeGAtestPoint at exit of active volume
   if (TVirtualMC::GetMC()->IsTrackExiting() || TVirtualMC::GetMC()->IsTrackStop() ||
      ...
      if (gMC->IsTrackExiting()) {
         ...
         std::cout << " AtSeGAtest: Track is exiting "
                   << "\n";
         LOG(INFO) << " SeGA: First hit in Volume " << fVolName;
         LOG(INFO) << " Particle : " << gMC->ParticleName(gMC->TrackPid());
         LOG(INFO) << " PID PdG : " << gMC->TrackPid();
...
void AtSeGAtest::EndOfEvent()
{

   fAtSeGAtestPointCollection->Clear();
}

void AtSeGAtest::Register()
{
   ...
   */
   ...
   FairRootManager::Instance()->Register("AtSeGAtestPoint", "AtSeGAtest", fAtSeGAtestPointCollection, kTRUE);
}

TClonesArray *AtSeGAtest::GetCollection(Int_t iColl) const
{
   if (iColl == 0) {
      return fAtSeGAtestPointCollection;
   } else {
      return nullptr;
   }
}

void AtSeGAtest::Reset()
{
   fAtSeGAtestPointCollection->Clear();
}

void AtSeGAtest::Print(Option_t *option) const
{
   Int_t nHits = fAtSeGAtestPointCollection->GetEntriesFast();
   LOG(INFO) << "SeGA: " << nHits << " points registered in this event";
}

void AtSeGAtest::ConstructGeometry()
{
...
Bool_t AtSeGAtest::CheckIfSensitive(std::string name)
{
...
AtSeGAtestPoint *AtSeGAtest::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
                             Double_t eLoss)
{
   TClonesArray &clref = *fAtSeGAtestPointCollection;
   Int_t size = clref.GetEntriesFast();
   return new (clref[size]) AtSiPoint(trackID, detID, pos, mom, time, length, eLoss);
}

// -----   Private method AddHit   --------------------------------------------
AtSiPoint *AtSeGAtest::AddHit(Int_t trackID, Int_t detID, TString VolName, Int_t detCopyID, TVector3 posIn,
                             ...
                             Double_t eLoss, Double_t EIni, Double_t AIni, Int_t A, Int_t Z)
{
   ...
   if (fVerboseLevel > 1)
      LOG(INFO) << "SeGA: Adding Point at (" << posIn.X() << ", " << posIn.Y() << ", " << posIn.Z()
...
std::pair<Int_t, Int_t> AtSeGAtest::DecodePdG(Int_t PdG_Code)
{
